...
- (void)startAlipay:(NSString *)_orderId price:(NSString*)_price withResult:(AlipayResultBlock)resultBlock
{
    ...
    order.partner       = [PLConfig sharedConfig].alipayPID;
    order.seller        = [PLConfig sharedConfig].alipaySellerID;
    ...
    order.notifyURL     =  [PLConfig sharedConfig].alipayNotifyURL; //回调URL
    ...
    NSString *appScheme = [PLConfig sharedConfig].alipayScheme;
//
    //获取私钥并将商户信息签名,外部商户可以根据情况存放私钥和签名,只需要遵循RSA签名规范,并将签名字符串base64编码和UrlEncode
    id<DataSigner> signer   = CreateRSADataSigner([PLConfig sharedConfig].alipayPrivateKey);
