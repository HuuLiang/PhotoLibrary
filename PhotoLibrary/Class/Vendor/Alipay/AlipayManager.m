...
//    
    ...
    order.partner       = [PLPaymentConfig sharedConfig].alipayInfo.partner;
    order.seller        = [PLPaymentConfig sharedConfig].alipayInfo.seller;
    
    order.tradeNO       = _orderId;         //订单ID（由商家自行制定）
    order.productName   = [PLPaymentConfig sharedConfig].alipayInfo.productInfo; //商品标题
    order.productDescription = PL_PAYMENT_RESERVE_DATA; //商品描述
    order.amount        = [NSString stringWithFormat:@"%.2f", _price/100.];           //商品价格
    order.notifyURL     =  [PLPaymentConfig sharedConfig].alipayInfo.notifyUrl; //回调URL
    ...
    NSString *appScheme = PL_ALIPAY_SCHEME;
//
    //获取私钥并将商户信息签名,外部商户可以根据情况存放私钥和签名,只需要遵循RSA签名规范,并将签名字符串base64编码和UrlEncode
    id<DataSigner> signer   = CreateRSADataSigner([PLPaymentConfig sharedConfig].alipayInfo.privateKey);
